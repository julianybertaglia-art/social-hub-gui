...
const PREVIEW_ID = 'juliany-menu-preview-20260918-v2';
const RECIPIENT = '5511972668922';

export async function POST(request) {
  const body = await request.json().catch(() => ({}));
  if (body?.action !== 'send-juliany-menu-preview') {
  ...
  if (!contact) {
    return Response.json({ ok: false, error: 'Contato de prévia não encontrado.' }, { status: 404 });
  }
  ...
  if (previousError) {
    return Response.json({ ok: false, error: previousError.message }, { status: 500 });
  }
  if (previous?.length) {
    return Response.json({ ok: true, alreadySent: true, message: previous[0] });
  }
