...
async function profileFor(participant) {
  const base = { id:String(participant?.id||''), username:participant?.username||'', name:participant?.name||'' };
...
export async function GET() {
  try {
    ...
    const payload = await metaRequest(
      `${identity.accountId}/conversations?fields=id,participants{id,username,name},updated_time,messages.limit(15){id,message,from,to,created_time}&limit=50`
    );
    ...
    for (const conv of recent) {
      ...
      const accountMentionedEvent = ordered.some((m) => (
        String(m?.from?.id || '') === identity.accountId
    ...
    return Response.json({ok:false,error:String(error?.message||error).slice(0,450)}, {status:Number(error?.status)||500});
  }
}
