...
export async function GET() {
  try {
    ...
    const rows = await loadLogs(supabase);
...
export async function POST(request) {
  ...
  try {
    ...
    if (!phone || ['Venda', 'Perdido'].includes(contact.stage) || !contactTags.includes(expectedTag)) {
      const reason = !phone
        ? 'Número inválido'
        : 'Lead não está mais elegível para esta campanha';
      ...
      return Response.json({
        ...
        skipped: true,
        reason,
        contact: { id: contact.id, name: contact.profile_name, phone },
      });
    }
    ...
    if (!asset?.data_base64) {
      throw new Error('Áudio ' + log.segment + ' não está salvo.');
    }
    ...
    const rows = await loadLogs(supabase);
    const currentCounts = counts(rows);

    return Response.json({
      ok: true,
      sent: true,
      done: currentCounts.pending === 0,
      contact: {
        id: contact.id,
    ...
    if (log?.id) {
      try {
        await supabase
          ...
          .update({
            status: 'failed',
            reason: error instanceof Error ? error.message : 'Falha ao enviar',
            updated_at: new Date().toISOString(),
          })
          .eq('id', log.id);
      } catch {}
